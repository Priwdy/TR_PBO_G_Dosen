...
import Controller.ControllerTambahDosen;
...
public class TambahDosen extends javax.swing.JFrame {   
    private final ControllerTambahDosen controller = new ControllerTambahDosen();
    ...
    private void load_table_data() { 
        DefaultTableModel model = controller.getDaftarDosen();
    ...
    private void jButton1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton1ActionPerformed
        String namaDsn = jTextField1.getText(); 
        String nid = jTextField2.getText();      
        
        if (namaDsn.isEmpty() || nid.isEmpty()) { 
            JOptionPane.showMessageDialog(this, "Nama dan NID harus diisi!", "Input Error", JOptionPane.WARNING_MESSAGE);
            return;
        }

        
        Dosen_UKSW dosenBaru = new Dosen_UKSW(nid, namaDsn);

        try {
            boolean success = controller.tambahDosen(dosenBaru);
            
            if (success) {
                JOptionPane.showMessageDialog(this, 
                    "Dosen " + dosenBaru.getNamaDosen() + " (NID: " + dosenBaru.getNID() + ") berhasil ditambahkan!\n" +
                    "(Password Default: dosen_123)", 
             ...
             if (e.getErrorCode() == 1062) { 
                 JOptionPane.showMessageDialog(this, "Gagal: NID " + nid + " sudah terdaftar.", "Error", JOptionPane.ERROR_MESSAGE);
             } else {
                 JOptionPane.showMessageDialog(this, "Gagal menambah dosen:\n" + e.getMessage(), "Error Database", JOptionPane.ERROR_MESSAGE);
             }
        }
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        ...
        int confirm = JOptionPane.showConfirmDialog(this, 
            "Yakin hapus data dosen NID: " + nidToDelete + "?\n(Akun login juga akan dihapus!)", 
            "Konfirmasi Hapus", 
            JOptionPane.YES_NO_OPTION);

        if (confirm == JOptionPane.YES_OPTION) {
            try {
                boolean success = controller.hapusDosen(nidToDelete);
                if (success) {
                    JOptionPane.showMessageDialog(this, "Data dosen berhasil dihapus.", "Berhasil", JOptionPane.INFORMATION_MESSAGE);
                    load_table_data();
                }
            } catch (SQLException e) {
